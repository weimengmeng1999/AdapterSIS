...
def eval_linear(args):
    ...
    seg_decoder = UNet(n_channels=3, n_classes=2)
...
def train(model, seg_decoder, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, target, idx) in metric_logger.log_every(loader, 20, header):
        ...
        inp_t2 = F.interpolate(inp, scale_factor=(1.5, 1.5), mode='bilinear', align_corners=False)
        inp_d2 = F.interpolate(inp, scale_factor=(0.5, 0.5), mode='bilinear', align_corners=False)
        with torch.no_grad():
          ...
          x_tokens_list_t2 = model(inp_t2)
        
          intermediate_output = x_tokens_list_t2[-1:]
          output_en_t2 = torch.cat([outputs for outputs, _ in intermediate_output], dim=-1)
          output_en_t2 = rearrange(output_en_t2, "b (h w) (p1 p2 c) -> b c (h p1) (w p2)", 
                             p1 = 1, p2 = 1, 
                             h = H*3 // 28, w = W*3 // 28, 
                             c = 384)
          
          x_tokens_list_d2 = model(inp_d2)
        
          intermediate_output = x_tokens_list_d2[-1:]
          output_en_d2 = torch.cat([outputs for outputs, _ in intermediate_output], dim=-1)
          output_en_d2 = rearrange(output_en_d2, "b (h w) (p1 p2 c) -> b c (h p1) (w p2)", 
                             p1 = 1, p2 = 1, 
                             h = H // 28, w = W // 28, 
                             c = 384)
          
        #   intermediate_output = x_tokens_list_d2[-3:-2]
        #   output_en_aff = torch.cat([outputs for outputs, _ in intermediate_output], dim=-1)
        #   output_en_aff = rearrange(output_en_aff, "b (h w) (p1 p2 c) -> b c (h p1) (w p2)", 
        #                      p1 = 1, p2 = 1, 
        #                      h = H // 28, w = W // 28, 
        #                      c = 384)
        # print(output_en.shape)
        output = seg_decoder(inp, output_en, output_en_t2, output_en_d2)
...
class UNet(nn.Module):
    def __init__(self, n_channels = 3, n_classes = 2, outplanes = 1024, embed_dim=384, dw_stride= 1, bilinear=False):
        ...
        # expansion = 4
        self.fuser = FusionModel()
        # self.expand_block_1 = FCUUp(inplanes=embed_dim, outplanes=512, up_stride=dw_stride)
        self.expand_block_2 = FCUUp(inplanes=embed_dim, outplanes=256, up_stride=dw_stride)
        self.expand_block_3 = FCUUp(inplanes=embed_dim, outplanes=128, up_stride=dw_stride)
        self.expand_block_4 = FCUUp(inplanes=embed_dim, outplanes=64, up_stride=dw_stride)

        # self.fusion_block = ConvBlock(inplanes=outplanes, outplanes=outplanes, groups=1)

    def forward(self, x, x_o, x_t2, x_d2):
        x1 = self.inc(x)
        _, _, H, W = x1.shape
        x_t2_r = self.expand_block_4(x_t2, H, W)
        x1 = self.fuser(x1, x_t2_r)

        x2 = self.down1(x1)
        _, _, H, W = x2.shape
        x_o_r = self.expand_block_3(x_o, H, W)
        x2 = self.fuser(x2, x_o_r)


        x3 = self.down2(x2)
        _, _, H, W = x3.shape
        x_d2_r = self.expand_block_2(x_d2, H, W)
        x3 = self.fuser(x3, x_d2_r)
